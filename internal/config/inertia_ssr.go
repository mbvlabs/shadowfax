...
// config/inertia.go: process environment (including .env), then DefaultInertiaSSR*
// constants from config/inertia.go.
func ReadSSRSettings(scaffold *ScaffoldConfig) (SSRSettings, error) {
	...
	settings := SSRSettings{
		Mode:    resolveSSRSetting("INERTIA_SSR_MODE", fileDefaults.Mode),
		URL:     resolveSSRSetting("INERTIA_SSR_URL", fileDefaults.URL),
		Bundle:  resolveSSRSetting("INERTIA_SSR_BUNDLE", fileDefaults.Bundle),
		Runtime: resolveSSRSetting("INERTIA_SSR_RUNTIME", fileDefaults.Runtime),
	}
